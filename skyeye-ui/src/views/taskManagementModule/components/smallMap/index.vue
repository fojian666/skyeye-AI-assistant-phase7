<template>
  <div style="position: relative">
    ...
    <div
      v-if="siteImagePath"
      class="site-photo-panel"
      :class="{ collapsed: sitePhotoCollapsed }"
    >
...
export default {
  ...
  methods: {
    ...
    updateSector(yaw) {
      ...
      const yawDegree = (this.currentTask && this.currentTask.yawDegree != null)
    ...
    hasValidPolygonCoords(polygonData) {
      ...
      if (geom.type === 'MultiPolygon') {
        ...
        return Array.isArray(firstPoly) && firstPoly.length > 0
          && Array.isArray(firstPoly[0]) && firstPoly[0].length > 0;
      }
    ...
    drawSupervisionPolygon(task) {
      ...
      if (polygon && this.hasValidPolygonCoords(polygon)) {
        ...
        if (bounds.isValid() && !this.overlay && !this.mapPanoramaMode && !this.polygonOnlyMode
          && this.mapViewMode !== 'vertical') {
          this.map.fitBounds(bounds, { padding: [30, 30], maxZoom: 18 });
          return;
        }
      }

      if (!this.overlay && !this.mapPanoramaMode && !this.polygonOnlyMode
        && this.mapViewMode !== 'vertical') {
    ...
    setMapZoom(zoom) {
      if (!this.map) return;
      const lat = this.currentTask.latitude != null
        ? this.currentTask.latitude
        : this.currentTask.lat;
      const lon = this.currentTask.longitude != null
        ? this.currentTask.longitude
        : this.currentTask.lon;
      if (lat != null && lon != null) {
    ...
    fitMapToSupervisionTask(task) {
      ...
      const lat = task.plotLatitude != null
        ? task.plotLatitude
        : (task.latitude != null ? task.latitude : task.lat);
      const lon = task.plotLongitude != null
        ? task.plotLongitude
        : (task.longitude != null ? task.longitude : task.lon);
      if (lat != null && lon != null) {
        this.safeSetView(lat, lon, 18);
      }
    },
