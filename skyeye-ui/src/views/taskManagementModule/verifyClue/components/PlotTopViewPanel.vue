<template>
    <div class="plot-top-view-panel" v-loading="loading">
        <div class="panel-toolbar">
            <el-button type="text" class="back-btn" @click="$emit('back')"> <i class="el-icon-arrow-left"></i> 返回列表 </el-button>
        ...
        <div v-if="hasPoint" class="panel-body">
            ...
            <div class="panel-view-tabs">
                <div :class="['view-tab', { active: activeTab === 'panorama' }]" @click="handleTabChange('panorama')">全景图</div>
                <div :class="['view-tab', { active: activeTab === 'vertical' }]" @click="handleTabChange('vertical')">俯视图</div>
            </div>
        ...
        <div v-else class="empty-panel">暂无项目详情</div>
    </div>
</template>

<script>
import { TASK_MGMT_USE_MOCK, getSupervisionPolygonDetailApi, buildSupervisionPolygonDetailParams } from '@/api/taskMgmtApi';
import { getPanoramaImageApi } from '@/api/commonApi';
import { mockGetSupervisionPolygonDetailApi, mockGetPanoramaImageApi, mockCalculatePanoramaApi } from '@/views/taskManagementModule/mock/taskApi';
...
export default {
    ...
    methods: {
        ...
        async loadPanoramaList(loadSeq, plotId) {
            ...
            const pointId = this.detailPointId || this.currentPoint.pointId || this.currentPoint.point_id;
            if (!pointId) {
        ...
        async loadTopViews(polygonId) {
            ...
            const fetchApi = TASK_MGMT_USE_MOCK ? mockGetSupervisionPolygonDetailApi : getSupervisionPolygonDetailApi;
            try {
                ...
                if (res.code === 0 && res.data) {
                    ...
                    this.siteImagePath = rawPath ? this.resolveImageUrl(rawPath) : DEFAULT_SITE_IMAGE_PATH;
                    ...
                    this.constructionDesc = res.data.construction_desc || res.data.constructionDesc || '';
