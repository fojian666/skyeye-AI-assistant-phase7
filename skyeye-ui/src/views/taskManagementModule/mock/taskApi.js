...
import { MOCK_TEMP_LAND_RESTORE_DETAIL, MOCK_MOUNTAIN_WATER_DETAIL, MOCK_CONSTRUCTION_DETAIL } from '@/views/taskManagementModule/mock/taskList';
...
function findMockTableRowById(id) {
    const all = [...MOCK_TEMP_LAND_RESTORE_TABLE, ...MOCK_MOUNTAIN_WATER_TABLE, ...MOCK_CONSTRUCTION_TABLE];
    return all.find((item) => String(item.id) === String(id) || item.task_id === String(id));
}

/** 模拟：监管项目列表/详情统一接口 */
export async function mockGetSupervisionProjectListApi(params = {}) {
    await delay();
    const { pageIndex = 1, pageSize = 10, id, dataType = 1, taskId } = params;
    const searchId = id != null && id !== '' ? id : taskId;

    if (searchId != null && searchId !== '') {
        const row = findMockTableRowById(searchId);
        const taskType = row ? row.task_type : DATA_TYPE_TO_TASK_TYPE[dataType] || TASK_TYPE.TEMP_LAND_RESTORE;
...
function resolveMockDetailPointId(polygonId) {
    const allPoints = [...MOCK_TEMP_LAND_RESTORE_DETAIL, ...MOCK_MOUNTAIN_WATER_DETAIL, ...MOCK_CONSTRUCTION_DETAIL];
    const matched = allPoints.find(
        (item) => String(item.id) === String(polygonId) || String(item.pointId) === String(polygonId) || String(item.point_id) === String(polygonId)
    );
...
export function getMockPanoramaListByPointId(pointId) {
    const allPoints = [...MOCK_TEMP_LAND_RESTORE_DETAIL, ...MOCK_MOUNTAIN_WATER_DETAIL, ...MOCK_CONSTRUCTION_DETAIL];
    const matched = allPoints.filter((item) => String(item.pointId) === String(pointId) || String(item.point_id) === String(pointId));
...
export async function mockGetSupervisionPolygonListApi(params = {}) {
    await delay();
    const { pageIndex = 1, pageSize = 10, id, supervisionProjectId, dataType = 1 } = params;
    const projectId = supervisionProjectId != null && supervisionProjectId !== '' ? supervisionProjectId : id;
