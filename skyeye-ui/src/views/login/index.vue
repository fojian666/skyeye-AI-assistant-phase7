<template>
    <div class="se-container-login">
                        ...
                        <div class="login-input">
                            <el-form :rules="rules" :model="formFields" ref="LoginForm">
                                ...
                                <el-form-item class="button-item">
                                    <el-button type="" class="login-btn" @click="loginBtnClick('LoginForm')"> 登录 </el-button>
                                </el-form-item>
                            </el-form>
                        </div>
                    </div>
        ...
        <div class="foot">版权所有：©空间感知与优化计算研究所.</div>
    </div>
</template>
...
export default {
    ...
    data() {
        ...
        let checkUsername = (rule, value, callback) => {
            if (!value || value.trim() == '') return callback(new Error('用户名不能为空'));
            if (value.trim().length > 16) return callback(new Error('用户名长度不能超过16位'));
            callback();
        };
