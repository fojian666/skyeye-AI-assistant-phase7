    ...
    <div>
        ...
        <div class="form-box-content">
            <a-form-model ref="ruleForm" :model="form" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol">
                ...
                <a-form-model-item label="前景影像" prop="prev_id" v-if="activeKey == '地类变化'">
                    <a-select v-model:value="form.prev_id" placeholder="请选择前景影像">
                        <a-select-opt-group v-for="items in resListsObj.imagesList">
                            <span slot="label" style="font-size: 16px; color: #42b4f2">
                                <i class="iconfont icon-groai-base" style="margin-right: 8px" />{{ items.name }}
                            </span>
                            <a-select-option v-for="item in items.values" :value="item.id" :disabled="form.county === items.name ? false : true">
                                {{ item.name }}
                            </a-select-option>
                        </a-select-opt-group>
                    </a-select>
                ...
                <a-form-model-item label="后景影像" prop="next_id" v-if="activeKey == '地类变化'">
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                ...
                prev_id: [{ required: true, message: '请选择一个影像！', trigger: 'change' }],
                next_id: [{ required: true, message: '请选择一个影像！', trigger: 'change' }],
                path_id: [{ required: true, message: '请选择一个影像！', trigger: 'change' }],
                data_path_id: [{ required: true, message: '请选择一个影像！', trigger: 'change' }],
                county: [
                    {
                        required: true,
    ...
    methods: {
        ...
        async resRegister() {
            // 判断前后时影像
            if (this.activeKey == '地类变化' && this.form.prev_id === this.form.next_id) {
