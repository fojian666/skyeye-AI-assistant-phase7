    ...
    <a-layout id="components-layout-demo-side">
      <a-layout-sider v-model="collapsed" collapsible v-if="showLeftBar">
        ...
        <a-menu mode="inline" @click="handleClick":openKeys="currentOpenKeys"  :selectedKeys="[$route.path]">
          <a-sub-menu
                      v-for="item in dataList" :key="item.url">
            <span slot="title">
              <i :class="'iconfont ' + item.icon" style="margin-right: 8px"
    ...
    '$route.path': {
      handler(newVal, oldVal) {
        if (newVal.includes(oldVal)) {
          if (
              newVal === '/resource-center/system-management/user-info'
          ) {
            this.resourceFlag = true;
          } else if (
              newVal === '/resource-center/task-directory/task-details'
          ) {
            this.taskFlag = true;
          }
        } else {
