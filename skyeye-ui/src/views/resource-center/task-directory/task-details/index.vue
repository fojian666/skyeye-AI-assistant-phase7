        ...
        <table style="table-layout: fixed; width: 100%">
          <tr>
            ...
            <td class="resource-detail-value" v-show="show">
              {{ dataObj.name }}
              <span
                  class="iconfont icon-geoai-edit card-title-font"
                  @click="toggle"
              />
            </td>
            <td class="resource-detail-value" v-show="!show">
          ...
          <tr v-if="task_type === '地类分割'">
            ...
            <td class="resource-detail-value">
              <span
                  ...
                  @click="previewaChievementSlicing"
              >
                预览
              </span>
            </td>
  ...
  methods: {
    async getTaskById() {
      ...
      if (res.code === 0) {
        ...
        if (this.dataObj.nextImage && this.dataObj.nextImage.url && this.dataObj.nextImage.url.includes('iserver') ||
    ...
    iserver() {
      ...
      if (this.task_type === '地类分割' || this.task_type === '目标检测') {
        ...
        this.axios
            .get(nextUrl + '.json', {withCredentials: false})
            .then((res) => {
              this.backgroundImage.query.center = res.data.center;
            });
      }
      this.axios.get(url + '.json', {withCredentials: false}).then((res) => {
    ...
    arcgis() {
      ...
      this.axios.get(url + '?f=pjson', {withCredentials: false})
          .catch((err) => {
            this.available = false;
          });
    },
