<template>
    <div class="content-box">
        <CpTab :cpTablList="taskList" :selectTab="taskSelectedTab" @cpTabItemClick="taskItemClick"></CpTab>
        <a-form-model :model="form" ref="ruleForm">
            <a-row>
                <a-col :span="7">
                    <a-form-model-item label="任务名称：" prop="name" :label-col="{ span: 5 }" :wrapper-col="{ span: 16 }">
                        <a-input placeholder="请输入" v-model="form.name" />
                    </a-form-model-item>
                </a-col>
                <a-col :span="7">
                    <a-form-model-item label="创建时间：" prop="createDate" :label-col="{ span: 5 }" :wrapper-col="{ span: 16 }">
                        <a-date-picker v-model="form.createDate" style="width: 100%" valueFormat="YYYY-MM-DD" placeholder="请选择时间" />
                    </a-form-model-item>
                </a-col>
                <a-col :span="6">
                    <a-form-model-item label="所属行政区：" prop="county" :label-col="{ span: 7 }" :wrapper-col="{ span: 16 }">
                        <a-select v-model="form.county" style="width: 100%" placeholder="请选择行政区">
                ...
                <a-col :span="2" class="btn">
                    <a-button type="primary" @click="search" class="search" icon="search">搜索 </a-button>
                </a-col>
                <a-col :span="2" class="btn">
        ...
        <div class="list-content">
            <div class="content-card" v-for="item in taskListsData" :key="item.id">
                ...
                <div @click="loadTaskDetails(item.id, item.taskType)">
                    ...
                    <p class="service" v-if="item.nextImage && item.nextImage.serviceType">服务类别：{{ item.nextImage.serviceType }}</p>
        ...
        <div class="pagination">
            <div class="pagination-describe">
                共 {{ total }} 条记录 第{{ params.pageIndex + '/' + Math.ceil(total / params.pageSize) }}
                页
            </div>
            <div class="page-number">
                <a-pagination show-quick-jumper v-model="params.pageIndex" :total="total" :page-size="params.pageSize" @change="handleChange" />
            </div>
        </div>
    </div>
</template>
