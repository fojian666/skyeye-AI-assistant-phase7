    ...
    <div class="authority-management">
        ...
        <div class="gt-breadcrumb-cnt">
            <div class="button-group">
                <a-input-search placeholder="请输入角色名称" class="search-input"
                                enter-button @search="onSearch" pressEnter="onSearch"/>
                ...
                <a-table :columns="columns" :data-source="tableData.dataSource" :loading="loading">
                    <template slot="operation" slot-scope="text, record">
                        <a-button-group class="role-operate-group">
                            <a-button style="background-color: #009688;border-color: #009688"
                                      @click="getMenusByRole(record.id)">关联菜单
                            </a-button>
                            <a-button style="background-color: #177DE4;border-color: #177DE4"
                                      @click="getUsersByRole(record.id)">关联用户
                            </a-button>
                            <a-button style="background-color: #ffb800;border-color: #ffb800"
                                      @click="editRoleInfo(record)">编辑
                            </a-button>
                            <a-button style="background-color: #ff5722;border-color: #ff5722"
                                      @click="deleteRoleConfirm(record)">删除
                            </a-button>
                        </a-button-group>
                    </template>
                </a-table>
        ...
        <a-modal v-model="newRole.model" :title="newRole.title" centered :footer="null" wrapClassName="rc-light-modal">
            <a-form-model class="add-role-form"
                          @submit="addNewRole"
                          ref="newRoleForm"
                          :model="newRole.form"
                          :rules="newRole.rules"
            >
                <a-form-model-item ref="name" label="名称" prop="name" name="name" required>
                    <a-input v-model:value="newRole.form.name" placeholder="请输入角色名称"
                             @blur="() => {$refs.name.onFieldBlur();}"/>
                </a-form-model-item>

                <a-form-model-item ref="abbreviation" label="缩写" prop="abbreviation" required>
                    <a-input v-model:value="newRole.form.abbreviation" placeholder="请输入缩写"
                             @blur="() => {$refs.abbreviation.onFieldBlur();}"/>
                </a-form-model-item>

                <a-form-model-item label="备注">
                    <a-input v-model:value="newRole.form.remark" type="textara"
                             @blur="() => {$refs.remark.onFieldBlur();}"/>
                </a-form-model-item>

                <a-form-model-item ref="order" label="排序号" prop="order" required>
                    <a-input v-model.number="newRole.form.order" placeholder="请输入排序号"
                             @blur="() => {$refs.order.onFieldBlur();}"/>
                </a-form-model-item>
        ...
        <!--    关联菜单-->
        <a-modal id="menuModel" width="720px" height="560px"
                 v-model="menuData.model" title="菜单权限管理" centered :footer="null" wrapClassName="rc-light-modal">
            <div class="menu-body">
                <div class="menu-btn">
                ...
                <div class="menu-tree">
                    <a-tree v-model="menuData.selectValue" checkable :replaceFields="menuData.replaceFields"
                            :defaultExpandedKeys="menuData.selectValue" :selected-keys="menuData.selectValue"
                            @check="check"
        ...
        <!--    关联角色-->
        <a-modal id="userModel" width="820px" height="540px"
                 v-model="userData.model" title="用户数据" centered :footer="null" wrapClassName="rc-light-modal">
            <div class="user-body">
                <div style="margin-bottom: 16px">
                    <a-button type="primary"
                              @click="setUsersByRoles" :disabled="!hasUserSelected">保存数据
                    </a-button>
              ...
              <template v-if="hasUserSelected">
                ...
                <a-table class="user-table"
                         rowKey="id" :columns="userData.userColums" :data-source="userData.data"
...
<script>
    ...
    import {
        ...
        getMenuRoleDataApi,
        getRoleDataApi,
        getUserRoleDataApi, postMenuRoleDataApi,
        postRoleDataApi,
        postUserRoleDataApi,
        putRoleDataApi
    } from "@/api/commonApi";
            ...
            return {
                ...
                newRole: {
                    ...
                    rules: {
                        ...
                        remark: [
                            {max: 40, message: '备注信息过长', trigger: 'blur'}
                        ]
                    }
                },
