<template>
	<div class="model-info">
		...
		<div class="gt-breadcrumb-cnt">
			<el-row :gutter="20" class="add">
				...
				<el-col :span="4">
					...
					<el-button type="primary" @click="addDialogVisible = true"><i
									class="iconfont icon-tianjia"></i>新增模型
					</el-button>
				</el-col>
			</el-row>
			<!--模型列表 -->
				...
				<el-table-column label="操作" align="center">
					<template v-slot="scope">
						...
						<el-button type="danger" icon="el-icon-delete" size="medium"
											 @click="removeModelData(scope.row.id)"></el-button>
					</template>
				</el-table-column>
			</el-table>
			...
			<!-- 添加模型对话框 -->
			<el-dialog title="添加模型" :visible.sync="addDialogVisible" width="50%" custom-class="rc-light-dialog" @close="addDislogClosed"
				...
				<el-form label-width="100px" ref="addFormRef" :model="addForm">
					...
					<el-form-item label="模型框架" prop="framework" placeholder="请选择模型框架">
						<el-select v-model="addForm.framework" style="width: 100%">
							<el-option v-for="item in frameworkList" :key="item" :label="item"
												 :value="item"></el-option>
						</el-select>
					</el-form-item>
					<el-form-item label="模型网络" prop="network" placeholder="请选择模型网络">
						<el-select v-model="addForm.network" style="width: 100%">
							<el-option v-for="item in networkList" :key="item.value" :label="item.name"
												 :value="item.value"></el-option>
						</el-select>
					</el-form-item>
					<el-form-item label="模型类型" prop="type">
						<el-select v-model="addForm.type" style="width: 100%" placeholder="请选择模型类型">
							<el-option v-for="item in modelTypeList" :key="item.value" :label="item.name"
												 :value="item.value"></el-option>
						</el-select>
					</el-form-item>
					...
					<el-form-item label="标签类别:" prop="labels">
						<el-select v-model="addForm.labelRels" placeholder="请选择标签类别" multiple clearable
											 style="width: 100%">
							<el-option v-for="item in labelList" :key="item.value" :label="item.name"
												 :value="item.value"></el-option>
						</el-select>
					</el-form-item>
          ...
          <el-button type="primary" @click="addModel">确 定</el-button>
			...
			<!-- 修改模型信息对话框 -->
			<el-dialog title="修改模型" @close="editClosed" :visible.sync="editDialogVisble" width="50%" custom-class="rc-light-dialog"
								 :modal-append-to-body="false">
				<el-form :model="editForm" ref="editFormRef" label-width="70px">
					...
					<el-form-item label="标签类别:" prop="labels">
						<el-select v-model="editForm.labelRels" placeholder="请选择标签类别" multiple clearable
											 style="width: 100%">
							<el-option v-for="item in labelList" :key="item.value" :label="item.name"
												 :value="item.value"></el-option>
						</el-select>
