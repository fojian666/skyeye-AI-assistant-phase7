<template>
  <div class="se-content-right-body">
      ...
      <div class="right-content-body">
        <div class="se-filter-form">
          ...
          <div>
            ...
            <el-tooltip :content="filterForm.regionZw.join(' / ')" placement="top"  :disabled="!filterForm.regionZw || filterForm.regionZw.length === 0">
              <el-cascader
                ref="cascaderRef"
          ...
          <div>
            ...
            <el-select v-model="filterForm.status" placeholder="请选择" style="max-width: 100px" clearable>
              <el-option v-for="item in batchStatusList" :key="item.value" :label="item.name" :value="item.value"
                         clearable>
              </el-option>
            </el-select>
          </div>
          <div>
            ...
            <el-select v-model="filterForm.batchType" placeholder="请选择" style="max-width: 100px" clearable>
              <el-option v-for="item in batchTypeOptions" :key="item.value" :label="item.name" :value="item.value"
                         clearable>
              </el-option>
            </el-select>
          </div>
            ...
            <el-table-column label="操作" align="center" width="200">
              <template slot-scope="scope">
                <ul class="action-list">
                  ...
                  <el-button type="text" size="mini" @click="finishVerify(scope.row)"
                             :disabled="isMarked(scope.row.orginalStatus)"
                  ><span :style="{ color: changeStyle(scope.row.orginalStatus) }">判读完成</span>
                  </el-button>
                  <el-button
                    type="text"
                    size="mini"
                    @click="exportBatchShp(scope.row)"
                    v-if="isMarked(scope.row.orginalStatus)"
                  ><span :style="{ color: showDownloadShpStyle(scope.row.orginalStatus) }">导出SHP</span>
                  </el-button>
                  <el-button type="text" size="mini" @click="handleSkipPath(scope.row)" v-if="scope.row.pointType === 0" style="margin-left: 10px">上传</el-button>
    ...
    <el-dialog title="新增任务" :visible.sync="dialogVisible" width="520px" top="100px" custom-class="custom-dialog"
               @close="resetForm">
      <el-form size="medium" :model="form" ref="form" :rules="rules" label-width="100px">
        <el-form-item label="开始日期:" prop="begindate">
          <el-date-picker v-model="form.begindate" type="date" placeholder="选择开始日期" format="yyyy-MM-dd"
                          value-format="yyyy-MM-dd">
          </el-date-picker>
        </el-form-item>
        ...
        <el-form-item label="区域选择:" prop="region">
          <el-cascader :options="regionOptions" v-model="form.region" :change-on-select="true"    ref="cascaderRefDialog"
