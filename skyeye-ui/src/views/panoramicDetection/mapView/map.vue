            ...
            <el-table :data="listData">
                ...
                <el-table-column property="operation" label="操作">
                    <template slot-scope="scope">
                        ...
                        <el-button type="text" size="small" @click="hanldeViewMultiLeft(scope.row)" :disabled="leftObj === scope.row"
                            >前景
                        </el-button>
                        <el-button type="text" size="small" @click="hanldeViewMultiRight(scope.row)" :disabled="rightObj === scope.row"
                            >后景
                        </el-button>
                    </template>
                </el-table-column>
...
import { getPanoramaImageApi, getMapInfoApi, getPanoramaPointByCountryApi } from '@/api/commonApi';
...
export default {
    ...
    data() {
        return {
            ...
            baseMap4528Proj:
                window.config.baseMap4528Proj || '+proj=tmerc +lat_0=0 +lon_0=120 +k=1 +x_0=40500000 +y_0=0 +ellps=GRS80 +units=m +no_defs',
    ...
    methods: {
        ...
        async init4528Map() {
            const { crs, layer, maxZoom, minZoom, fitBounds, initialZoom } = await load4528BaseMap(this.baseMapService, this.baseMapServiceType, {
            ...
            if (this.mapService != '') {
                ...
                await this.axios
                    ...
                    .then((res) => {
                        ...
                        this.mapBounds = [
                            [leftBottom.y, leftBottom.x],
                            [rightTop.y, rightTop.x]
                        ];
                    })
                    .catch((err) => {
                        console.error(err);
