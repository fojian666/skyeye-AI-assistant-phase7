            ...
            <ul>
                <li v-for="(item, index) in $store.state.currentMenuList" :title="item.caption" :key="item.url">
                    <div :class="{ active: currentIndex === index }" class="side-bar">
                        <router-link :to="item.url" class="planUrl">
...
export default {
    ...
    computed: {
        currentIndex() {
            return this.$store.state.currentMenuList.findIndex((menu) => menu.url === this.$route.path);
        },
        showLeftBar() {
...
.se-container .se-universal-left .planUrl {
    width: 100%;
    height: 100%;
    display: flex;
...
.se-container .se-universal-left .panoramaTitle {
    font-size: 12px;
    color: #fff;
    text-align: center;
