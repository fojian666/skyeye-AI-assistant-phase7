    ...
    <div id="toolbar">
      <a-menu mode="vertical">
        ...
        <a-menu-item title="图层列表" style="display: none">
          <a-dropdown>
            <a class="ant-dropdown-link" @click="e => e.preventDefault()">
              <i class="iconfont icon-geoai-list"></i></a>
        ...
        <a-menu-item style="display: none">
          <a-dropdown>
            <a class="ant-dropdown-link" @click="e => e.preventDefault()">
              <i class="iconfont icon-geoai-model"></i
              >{{ currentSelect.modelName }}</a
            >
            <a-menu slot="overlay" style="min-width: 8rem">
              <a-menu-item class="btn-active">
                <i class="iconfont icon-geoai-construction item-icon"></i
                >建设用地
              </a-menu-item>
            </a-menu>
          </a-dropdown>
...
export default {
  ...
  methods: {
    initMap() {
      ...
      // 添加卷帘图层
      this.showLayer.leftLayer =new TiledMapLayer(this.layersList.prev_url, {noWrap: true})
          .addTo(this.map);
      this.showLayer.rightLayer = new TiledMapLayer(this.layersList.next_url, {noWrap: true})
          .addTo(this.map);

      this.showLayer.compareLayer = L.control
          .sideBySide(this.showLayer.leftLayer, this.showLayer.rightLayer)
          .addTo(this.map);
    ...
    inference() {
      ...
      // 防止this失效
      let than = this;
      overlayAnalystService.overlayAnalysis(datasetOverlayAnalystParameters,
          function (serviceResult) {
            ...
            if (serviceResult.type == 'processCompleted' &&
              ...
              than.predictLayer = L.layerGroup([
                than.predictLayer.resultLayer
              ]);
