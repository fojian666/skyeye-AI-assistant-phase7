<template>
    <div style="width: 100%; height: 100%; display: flex">
        ...
        <div
            ...
            :style="{ position: 'relative', height: '100%', width: showSmallMap ? '50%' : '100%' }"
            @mousemove="mouseMoveFunction"
            @mouseleave="MouseLeaveFunction">
            <div v-if="embedMode" class="embed-map-toggle" @click="toggleEmbedMap" :title="showSmallMap ? '收起地图' : '展开地图'">
                <i :class="showSmallMap ? 'el-icon-arrow-left' : 'el-icon-arrow-right'"></i>
                <span>{{ showSmallMap ? '收起' : '地图' }}</span>
            </div>
            ...
            <div class="gt-toolbar-right gt-toolbar-embed" v-else>
                <div v-if="isAutoRotating" class="gt-alarms-list" title="暂停自动播放" @click="autoPlay">
                    <img src="@/assets/images/stop.png" />
                </div>
                <div v-else class="gt-alarms-list" title="自动播放" @click="autoPlay">
...
import { getOnePointMultiComInfoApi, getBufferLayerApi, getPointBufferLayerApi, getEnumOptionApi } from '@/api/commonApi';
...
export default {
    ...
    methods: {
        ...
        handleResetViewYaw() {
            ...
            if (this.$refs.smallMap) {
                this.$refs.smallMap.resetMapZoom();
            }
        },
        resizePanoramaOverlay() {
        ...
        toggleEmbedMap() {
            ...
            this.$nextTick(() => {
                if (this.showSmallMap && this.$refs.smallMap) {
                    if (this.$refs.smallMap.map) {
                        this.$refs.smallMap.map.invalidateSize();
                    }
                    this.$refs.smallMap.setMapZoom(18);
                }
                requestAnimationFrame(() => {
                    this.resizePanoramaOverlay();
                });
            });
        },
        //处理自动播放远近功能
        ...
        async handleViewerLoad() {
            ...
            this.viewer = pannellum.viewer('panoramaContainer', {
                ...
                multiRes: {
                    basePath: '/panoramaUrl/static/layers/' + (this.batchNumber || this.currentTask.batchId) + '/' + this.currentTask.imageId,
                    path: '/%l/%s%y_%x',
    ...
    async mounted() {
        ...
        this.pointId = this.embedMode ? 'embed' : this.$route.query.pointId;
        if (!this.embedMode) {
            this.getBufferGD();
        }
        this.showSmallMap = !this.embedMode;
...
.embed-map-toggle {
    ...
    &:hover {
        background: rgba(43, 179, 244, 0.85);
    }
