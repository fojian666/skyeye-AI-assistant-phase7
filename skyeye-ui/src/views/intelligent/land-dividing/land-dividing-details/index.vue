<template>
    <div class="all-polygon-container" v-if="isLoaded" v-loading="loading" element-loading-text="图斑生成中，请等候……"
...
<!--                <div class="header-border"></div>-->
                <div class="header-content">
                    ...
                    <div class="header-item">
                        {{ mapData.county }}，{{
                            mapData.path.appendTime
                        }}，地类分割图斑发现{{ mapData.count }}处<span
                            class="text-bold"
        ...
        <div class="details-right" id="details-map" v-loading="shpLoading" element-loading-text="推送shp中，请等候……"
             element-loading-spinner="el-icon-loading" element-loading-background="rgba(255,255,255,0.5)"></div>
    ...
    export default {
        ...
        methods: {
            ...
            initMap(mapData) {
                this.axios
                    ...
                    .get(mapData.path.url + `/collections/${mapData.path.collectId}/items/${mapData.path.tifId}`+'.json', {withCredentials: false})
                        ...
                        let transCenter = {};
                        if (
                            this.coordinate_system === "4326" ||
                            this.coordinate_system === "4490"
                        ) {
                            this.crs = L.CRS.EPSG4326;
                            this.crs.resolutions = [0.7031249999891485, 0.35156250000645817, 0.17578124999134512, 0.08789062499567256, 0.04394531250972024, 0.02197265625486012, 0.01098632812743006, 0.00549316406371503, 0.002746582031857515, 0.0013732910159287575, 6.866454960804162E-4, 3.4332275992417075E-4, 1.7166136807812276E-4, 8.583068403906138E-5, 4.291534201953069E-5, 2.1457682893727956E-5, 1.0728841446863978E-5, 5.364420723431989E-6, 2.6822103617159945E-6, 1.3411051808579973E-6, 6.705407064663857E-7, 3.3527035323319285E-7, 1.6763517661659642E-7]
            ...
            loadMarkerCluster(map, mapData) {
                ...
                L.supermap
                    .featureService(mapData.dataPath)
            ...
            loadGeoJSONLayer(map, mapData) {
                ...
                L.supermap
                    .featureService(mapData.dataPath)
                    .getFeaturesByBounds(boundsParam, (serviceResult) => {
                        let result = serviceResult.result.features;
                        if (!result || !result.features || result.features.length < 1) {
                            return;
                        }
            ...
            queryFeatures(DLFGLB) {
                let sqlParam = new SuperMap.GetFeaturesBySQLParameters({
                    ...
                    datasetNames: [
                        `${this.mapData.datasourceName}:${this.mapData.datasetsName}`
                    ],
                    toIndex: -1,
                    maxFeatures: -1
                });
                new FeatureService(this.mapData.dataPath)
                    .getFeaturesBySQL(sqlParam, (serverResult) => {
                        ...
                        for (let j = 0; j < this.tempArr.length; j++) {
                            ...
                            if (existingItem) {// 如果已存在，则数量加1
                                existingItem.count += 1;
                            } else {// 如果不存在，则添加新项，数量初始化为1
                                dataTypeList.push({
            ...
            layerDialog() {
                if (this.layerOptions.length === 0)
                    return this.$message.warning("没有叠加的图层！");
                this.flag = !this.flag;
            },
            pushShp() {
                ...
                this.axios
                    .post(config.BASE_URL + "common/upload_path", params)
                    .then((res) => {
            ...
            onChange(e, url, datasetsName, datasourceName) {
                this.axios(
                    `${url}/datasources/${datasourceName}/datasets/${datasetsName}/features.json`
                ).then((res) => {
                    this.overlayLayer(
                        e,
                        url,
                        res.data.startIndex,
                        res.data.featureCount,
                        datasets_name,
                        datasource_name
                    );
                });
            ...
            polygonAnalysis(record) {
                ...
                L.supermap
                    .featureService(this.mapData.dataPath)
                    .getFeaturesByIDs(idsParam, (serverResult) => {
                        let center = this.getCenter(
                            serverResult.result.features.features[0].geometry.coordinates[0][0]
                        );
                        const transCenter1 = this.crs.unproject(
                            L.point(center[1], center[0])
                        );
                        this.map.setView(
                            [transCenter1.lat, transCenter1.lng],
                            this.polygonZoom
                        );
...
<style scoped lang="scss">
  ...
  .toolbox li:nth-child(2), .arcgis-toolbox li:nth-child(2) {
    border-top: 1px solid #CCCCCC;
    border-bottom: 1px solid #CCCCCC;
  }

  .toolbox li:nth-child(3), .arcgis-toolbox li:nth-child(3) {
    border-bottom: 1px solid #CCCCCC;
  }
