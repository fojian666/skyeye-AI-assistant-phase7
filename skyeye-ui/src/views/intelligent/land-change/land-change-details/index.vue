...
<!--                <div class="header-border"></div>-->
                <div class="header-content">
                    ...
                    <div class="header-item">
                        {{ mapData.county }}{{ mapData.nextImage.appendTime }}和{{
                            mapData.prevImage.appendTime
                        }}影像变化检测结果，共计{{ mapData.count }}处变化图斑，分布于
        ...
        <div
                ...
                element-loading-background="rgba(255,255,255,0.5)"
        >
            <div class="ai-banner-title title-prev" ref="prev">
                前景时间：{{ mapData.prevImage.appendTime}}
            </div>
            <div class="ai-banner-title title-next" ref="next">
                后景时间：{{ mapData.nextImage.appendTime}}
            </div>
        </div>
        ...
        ></span>
            ...
            <li>
        <span
                class="icon iconfont icon-icon-line-graph"
                title="测距"
                @click="measureLength"
                id="distance"
        ></span>
            ...
            <li>
        <span
                class="icon iconfont icon-duobianxing"
                title="测面积"
        ...
        ></span>
            ...
            <li>
        <span
                class="icon iconfont icon-qingchu"
                title="清除"
                @click="clearDraw"
                id="clear"
        ></span>
...
<!--            </div>-->
        ...
        <DownloadOption
                v-show="reportFlag"
                @confirm="reportGenerator"
                @close="close"
                :treeDataList="mapData"
        ></DownloadOption>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            initSuperMap() {
                this.axios
                    .get(this.mapData.prevImage.url + `/collections/${this.mapData.prevImage.collectId}/items/${this.mapData.prevImage.tifId}`+'.json', {withCredentials: false})
                    .then((res) => {
                        ...
                        L.geoJSON(this.mapData.geojson, {
                            ...
                            onEachFeature: function (feature, layer) {
                                layer.bindPopup("ID: <a onclick='seePolygon(" + feature.properties.SMID + ")'>" + feature.properties.SMID +
                                    "</a><br>行政区:" + feature.properties.XZQ +
                                    "<br>经度:" + feature.properties.ZXDX +
                                    "<br>纬度:" + feature.properties.ZXDY +
                                    "<br>变化前地类::" + feature.properties.BHQDL +
                                    "<br>变化后地类::" + feature.properties.BHHDL);
                            }
                        }).addTo(this.map)
                        //监听卷帘移动事件，改变左右影像标题位置
            ...
            reportGenerator(optionList) {
                ...
                this.axios(config.BASE_URL + 'common/report_generator', {params}).then(
                    res => {
                        if (!res.data.status) {
            ...
            // 获取每一个行政区内图斑的SMID
            getSMID() {
                let geojsonURL = this.mapData.dataPath + '/datasources/' + this.mapData.datasourceName + '/datasets/' + this.mapData.datasetsName + '/features.geojson?fromIndex=0&toIndex=' + this.mapData.count;
