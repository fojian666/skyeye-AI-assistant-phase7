<template>
    <div class="one-polygon-container">
        <div class="details-left">
                ...
                <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" label-position="left">
                    <div class="form-content">
                        <el-form-item label="行政区:" prop="region">
                            <el-select v-model="ruleForm.region" clearable placeholder="请选择行政区">
                        ...
                        <el-form-item label="变化前地类:" prop="landTypeBeforeChange">
                            <el-select v-model="ruleForm.landTypeBeforeChange" clearable placeholder="请选择变化前地类">
                                <el-option v-for="item in landBeforeOptions" :key="item.value" :label="item.label" :value="item.value"> </el-option>
                            </el-select>
                        </el-form-item>
                        <el-form-item label="变化后地类:" prop="landTypeAfterChange">
                            <el-select v-model="ruleForm.landTypeAfterChange" clearable placeholder="请选择变化前地类">
                                <el-option v-for="item in landAfterOptions" :key="item.value" :label="item.label" :value="item.value"> </el-option>
                            </el-select>
                        </el-form-item>
                    </div>
                    <div class="btn-content">
                        <el-form-item>
                            ...
                            <el-button size="mini" class="reset" @click="resetForm">重置 </el-button>
                        </el-form-item>
                    </div>
                </el-form>
            </div>
            ...
            <VirtualList :cindex="this.currentSmid" :listData="listData" class="virtual-list" @changePolygon="changePolygon"></VirtualList>
        </div>
        <div class="details-right">
            <div class="map-container">
                <div id="map1" class="left-map">
                    <ul class="toolbox">
                        <li>
                            <span class="icon iconfont icon-gantanhao" title="查属性" @click="viewLeftProperty"></span>
                        </li>
                    </ul>
                </div>
                <div id="map2" class="right-map">
                    <ul class="toolbox">
                        <li>
                            <span class="icon iconfont icon-gantanhao" title="查属性" @click="viewRightProperty"></span>
                        </li>
                    </ul>
                </div>
                <div class="map-divider"></div>
                <div class="ai-banner-title title-prev">前景时间：{{ imagesAppendTime[0] }}</div>
                <div class="ai-banner-title title-next">后景时间：{{ imagesAppendTime[1] }}</div>
            </div>
                    ...
                    <template v-for="(item, index) in info.sourceCounty">
                        <div class="img" ref="img">
                            ...
                            <a-radio-group
                                ...
                                <a-radio :value="0" :disabled="item.disabled[0]">前景影像 </a-radio>
                                <!-- <div class="img-btn-diviver"></div> -->
                                <a-radio :value="1" :disabled="item.disabled[1]">后景影像 </a-radio>
                            </a-radio-group>
                        </div>
                    </template>
                </div>
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                region: [{ required: true, message: '请选择行政区', trigger: 'change' }],
                landTypeBeforeChange: [{ required: true, message: '请选择变化前地类', trigger: 'change' }],
                landTypeAfterChange: [{ required: true, message: '请选择变化后地类', trigger: 'change' }]
            },
            regionOptions: [],
            landBeforeOptions: [],
            landAfterOptions: [],
    ...
    methods: {
        ...
        initSuperMap() {
            ...
            this.axios
                .get(this.info.image1.url + `/collections/${this.info.image1.collectId}/items/${this.info.image1.tifId}` + '.json', {
        ...
        submitForm() {
            this.$refs.ruleForm.validate((valid) => {
                ...
                const landChangeArr = this.$route.query.component.listData.filter((item) => {
                    return (
                        item.title.includes(this.ruleForm.region) &&
                        item.title.includes(this.ruleForm.landTypeBeforeChange, item.title.indexOf('--') + 1) &&
                        item.title.includes(this.ruleForm.landTypeAfterChange, item.title.indexOf('>'))
                    );
                });
        ...
        viewLeftProperty() {
            ...
            //标记绑定弹窗显示
            this.leftLineMarker
                .bindPopup('<div>面积：' + parseFloat(this.polygonArea).toFixed(3) + ' ㎡</div>' + '<div>经纬度：' + this.latLng + '</div>')
                .openPopup();
        },
        viewRightProperty() {
            //添加一个标记
        ...
        async loadAllThumbnails() {
            ...
            await Promise.all(
                this.info.sourceCounty.map(async (item) => {
                    ...
                    try {
                        const res = await this.axios.get(`${item.url}/collections/${item.collectId}/items/${item.tifId}.json`, {
                            withCredentials: false
                        });
                        // 存储到缓存
        ...
        async getThumbnail(url, collectionId, tifId) {
            ...
            try {
                const res = await this.axios.get(`${url}/collections/${collectionId}/items/${tifId}.json`, { withCredentials: false });
