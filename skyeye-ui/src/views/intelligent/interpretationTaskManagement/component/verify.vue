...
export default {
    ...
    computed: {
        ...
        status: function () {
            if (this.currentStep === 0) return ['In Progress', 'Waiting', 'Waiting', 'Waiting'];
            else if (this.currentStep === 1) return ['Finished', 'In Progress', 'Waiting', 'Waiting'];
            else if (this.currentStep === 2) return ['Finished', 'Finished', 'In Progress', 'Waiting'];
            else if (this.currentStep === 3) return ['Finished', 'Finished', 'Finished', 'In Progress'];
        }
    },
    mounted() {
        this.isLoading = true;
        ...
        handleContinue() {
            if (this.currentStep === 0) {
                ...
                setTimeout(() => {
                    ...
                    this.$emit('continueTranslation', this.taskIsEmpty);
                }, 1500);
            }
        },
        handleCancel() {
            ...
            await getRedisCountApi()
                .then(async (response) => {
                    if (response.data.error) {
                        ...
                        this.allStep[1].text.push(
                            'redis中存在其他正在运行的项目，继续执行新建的项目，不会打开解译窗口，需要等待其他项目的任务执行完毕后在任务管理中的才能查看解译窗口，是否继续？'
                        );
                    }
                })
                .catch((error) => {
            ...
            await getGpuFreeMemoryApi()
                .then(async (response) => {
                    if (response.data.error) {
                        ...
                        this.allStep[2].text.push(
                            '请确保GPU剩余显存大于5GB，剩余物理内存大于20GB并且CPU利用率小于50%，检测到下列ip地址的硬件没有满足条件，继续运行任务会失败，是否继续？'
                        );
                        messageList.forEach((message) => {
                            this.allStep[2].text.push(
                                `ip地址：${message.ip_address}；剩余显存："${message.gpu_free_memory}"；剩余内存："${message.available_memory}"；CPU利用率："${message.used_cpu}"`
                            );
                        });
                    }
                })
                ...
                .then(async (response) => {
                    if (response.data.status === 'success') {
                        if (response.data.largedataCount !== 0 || response.data.errorEpsgcount !== 0) {
                            ...
                            this.allStep[3].text.push(
                                `数据中存在大范围数据或错误epsg数据，坐标系请转换为WGS_84或者CGCS2000坐标系;部分数据长宽大于${dataSize}，鉴定为大数据量，检测结果可能会出现内存溢出问题，导致检测失败，是否继续？`
                            );
                            messageList.forEach((message) => {
                                this.allStep[3].text.push(
                                    `${message.dataMessage}；坐标系："${message.epsg}"；长度："${message.rasterXSize}"；宽度："${message.rasterYSize}"`
                                );
                            });
                        }
                    } else {
