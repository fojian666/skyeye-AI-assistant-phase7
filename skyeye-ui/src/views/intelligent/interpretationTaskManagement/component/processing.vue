<template>
  <el-dialog id="interpreting-processing-dialog" title="进度信息" :close-on-click-modal="false" :visible.sync="detailVisible" width="90%" height="90%">
          ...
          <el-col :span="20">
            <div class="grid-content bg-purple-light" @mouseout='mouseout' @mouseover='mouseover'>
              ...
              >
                <el-table-column
                  prop="node_type"
                  label="节点类型" width="100">
                </el-table-column>
                <el-table-column
                  prop="name"
                  label="节点IP" width="150">
                </el-table-column>
                <el-table-column
                  prop="cpu"
                  label="CPU"
        ...
        <el-row v-for="(node_data, index) in nodes_data" :key="index">
          <el-col :span="24">
            ...
            <el-steps :active="node_data.node_information_all_step" align-center>
              <el-step :title="step.node_name" v-for="(step, index) in node_data.node_information" :key="index" :process-status="getStatus(step.status)"></el-step>
            </el-steps>
          </el-col>
        </el-row>
      </div>
...
export default {
  ...
  methods: {
    ...
    // 完成解译提示框
    allOverDialog() {
      this.$alert(this.detection_type + '已完成!!' + "成功：" + this.success_count + "失败："+this.error_count,
          '遥感解译', {
        confirmButtonText: '确定',
        callback: action => {
          //点击确定，清除定时器，停止请求
