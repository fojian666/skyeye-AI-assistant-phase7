    ...
    <div class="se-container">
            ...
            <div class="left-content-body">
                <div class="left-form">
                    ...
                    <div class="t_items">
                        ...
                        <el-select v-model="form.selectedDetecT" placeholder="请选择" clearable
                                   style="width: 220px">
                            <el-option v-for="item in detecTypesAll" :key="item.value" :label="item.label"
                    ...
                    <div class="t_items" v-if="!isShowChange">
                        ...
                        <el-select v-model="form.inputPath" placeholder="请选择服务器文件" clearable style="width: 220px">
                            <el-option v-for="item in serverPaths" :key="item" :label="item"
                    ...
                    <div class="t_items">
                        ...
                        <el-input v-model="form.savedPath" style="width: 220px"
                                  placeholder="结果默认存在共享文件下result文件夹中">
                        </el-input>
                    </div>
                    <div class="t_items">
                          <span slot="label" class="gt-main-label">
                           碎斑阈值：
                         </span>
                        <el-input-number v-model="form.threshold" controls-position="right" :min="1" :max="150"
                                         :step="1"
                                         style="width: 220px">
                        </el-input-number>
                    </div>
                    <div class="t_items">
                        ...
                        <el-select v-model="form.selectedMoldelT" placeholder="请选择" clearable
                                   style="width: 220px">
                            <el-option v-for="item in filterModelTypes" :key="item.id" :label="item.model_type_name"
                                       :value="item.model_type_name">
                    ...
                    <div class="t_items" style="justify-content: center">
                            ...
                            <el-button type="primary" @click="startDataVerify" size="mini" class="gt-main-form-item-btn">开始解译
                            </el-button>
                    </div>
                </div>
            </div>
        ...
        <div class="right-content">
            ...
            <div class="right-content-body">
                ...
                <staticTable
                        v-else
                        :modelTypes="modelTypes" :detectionTypeList="detecTypesAll"
                        ref="staticTable"
                ></staticTable>
            </div>
        </div>
        <verify title="验证窗口"  :form="form" :resultPaths="resultPaths" v-if="openVerifyDialog"  @closeDialog="openVerifyDialog=false" @continueTranslation="startTranslation"/>
        <!-- 开始解译后显示过程组件 -->
    </div>
...
export default {
    ...
    computed:{
        ...
        filterModelTypes(){
            if (this.modelTypes){
                if (this.form.selectedDetecT==='基于地类变化模型预测')
                    return this.modelTypes.filter(item => item.model_type==='地类变化')
    ...
    methods: {
        ...
        emptyRedis(){
            this.$confirm('此操作将清空所有Redis任务，是否继续','提示',{
                ...
                axios.post(config.BASE_URL+"apps/emptyRedis")
                    .then((response) => {
