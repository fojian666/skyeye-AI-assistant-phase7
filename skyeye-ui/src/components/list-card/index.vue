...
<!--              <img :src="thumbnailCache[`${item.nextImage.collectId}_${item.nextImage.tifId}`]" alt="缩略图"/>-->
          ...
          <div class="card-right">
            ...
            <p :title="item.county">
              <i class="list-card-icon iconfont icon-weizhi"></i>检测区域：<span style="color: #00f1f3">{{
                item.county
                }}</span>
            </p>
...
<!--            </p>-->
              <p :title="item.taskType">
                  <i class="list-card-icon iconfont icon-weizhi"></i>检测类型：<span style="color: #00f1f3">{{
                      item.taskType
                  }}</span>
              </p>
...
export default {
  ...
  async created() {
    ...
    this.listCardConfig.detailPath == '/intelligent/land-change/land-change-details'
      ? (this.flag = true)
      : (this.flag = false);
            ...
            await Promise.all(
                this.listCardConfig.data.map(async (item) => {
                    if (item.taskType === '地类变化'){
                        ...
                        try {
                            const res = await this.axios.get(
                                `${item.path.url}/collections/${item.path.collectId}/items/${item.path.tifId}.json`,
                                { withCredentials: false }
                            );
                            // 存储到缓存
                            this.$set(this.thumbnailCache, cacheKey, res.data.assets.thumbnail.href);
