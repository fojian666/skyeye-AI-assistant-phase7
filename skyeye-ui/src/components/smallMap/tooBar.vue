<template>
    <div>
        <div class="toolbar-management">
            <div class="draw-content" v-if="!isCollapsedToolBar">
                <div @click="changeBarIndex(1)" :class="{ activateManagebar: activeToolIndex === 1 }" v-if="!oneMapSingle">
                    <img src="@/assets/images/mapbar/ditu2.png" /><span class="toolbar-text">标记</span>
                </div>
                <span style="color: #cccccc" v-if="!oneMapSingle">|</span>
                <div @click="changeBarIndex(2)" :class="{ activateManagebar: activeToolIndex === 2 }">
                    <img src="@/assets/images/mapbar/gjx.png" /><span class="toolbar-text">工具</span>
                </div>
                <span style="color: #cccccc">|</span>
                <div @click="openCounty" v-show="isShowArea">
                    <span class="icon iconfont icon-map-marker icon-toolbar"></span>
                    <span class="toolbar-text">区划定位</span>
                </div>
                <span style="color: #cccccc">|</span>
                <div @click="resetBar">
                    <span class="icon iconfont icon-qingchu icon-toolbar"></span>
                    <span class="toolbar-text">清除</span>
                </div>
                <span style="color: #cccccc">|</span>
            </div>
            <div>
                <i class="el-icon-caret-right" title="展开" v-if="isCollapsedToolBar" @click="handleIsCollapsedToolBar"></i>
                <i class="el-icon-caret-left" title="收起" v-else @click="handleIsCollapsedToolBar"></i>
            </div>
        </div>
        ...
        <div
            ...
            <div :style="{ display: !isCollapsed ? 'block' : 'none' }">
                <div class="mark-tool-content">
                    ...
                    <div class="mark-tool" title="添加面" :class="{ drawbaractive: drawIndex === 3 }" @click="handleDrawPolygon">
                        <img src="@/assets/images/mapbar/polygon.png" />
                    </div>
                ...
                <div class="layer-style-setting">
                    ...
                    <div class="style-setting-panel" :style="{ display: !layerIsCollapsed ? 'block' : 'none' }">
                        ...
                        <div class="setting-item">
                            ...
                            <el-slider v-model="shapeOption.fillOpacity" :min="0" :max="1" :step="0.1" style="flex: 1"></el-slider>
                        </div>
                        <!-- 颜色选择 -->
                        ...
                        <div class="setting-item">
                            ...
                            <el-slider v-model="shapeOption.opacity" :min="0" :max="1" :step="0.1" style="flex: 1"></el-slider>
                        </div>
                        <!-- 边线宽度 -->
                        <div class="setting-item">
                            <label class="setting-label">边线宽度</label>
                            <el-slider v-model="shapeOption.weight" :min="0" :max="20" :step="1" style="flex: 1"></el-slider>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="mark-item">
                                <div class="top-item">
                                    <div class="top-item-left">
                                        ...
                                        <span class="plot-name" @click="handlePylogonLocation(item, event)">{{ item.plotName }}</span>
                                    </div>
                                    <div class="marker-edit">
                                        ...
                                        <el-button
                                            ...
                                            icon="el-icon-edit"
                                            title="编辑"
                                            @click="handleEditPolygon(item)"
                                            class="confirm-btn"></el-button>
                                        <el-button
                                            type="info"
                                            icon="el-icon-delete"
                                            title="删除"
        ...
        <div class="area-selector-popup" v-show="isShowCounty">
            ...
            <div class="popup-content">
                ...
                <div class="areas-grid">
                    <div v-for="(area, index) in areas" :key="index" class="area-item" @click="selectArea(area)">
                        {{ area }}
                    </div>
                </div>
            </div>
        ...
        <el-dialog
            ...
            <el-form class="form_class" :rules="formRules">
                <el-form-item label="名称:" style="display: flex" prop="plotName">
                    <el-input v-model="currentAddPolygon.plotName" class="select-item"></el-input>
                </el-form-item>

                <el-form-item label="备注:" style="display: flex" prop="plotDesc">
                    <el-input v-model="currentAddPolygon.plotDesc" class="select-item"></el-input>
                </el-form-item>

                <el-form-item label="线索:" style="display: flex" prop="clueId">
                    <el-input v-model="currentAddPolygon.clueId" class="select-item" disabled> </el-input>
                </el-form-item>
                <div class="btn-uploading">
...
export default {
    ...
    data() {
        return {
            // 表单验证规则
            formRules: {
                plotName: [{ required: true, message: '名称不能为空', trigger: 'blur' }],
                plotDesc: [{ required: true, message: '备注不能为空', trigger: 'blur' }]
            },
            countyService: '',
            datasource_name: 'NJXZQ',
            datasets_name: 'xzq_new',
            isShowCounty: false, // 控制弹窗显示
            areas: ['鼓楼区', '玄武区', '秦淮区', '建邺区', '栖霞区', '雨花台区', '浦口区', '江宁区', '六合区', '溧水区', '高淳区', '江北新区'], // 区域列表（可从接口动态获取）
            targetDivPosition: { x: 0 }, // 初始 X 轴位置
            activeToolBarIndex: 0,
            activeToolIndex: this.oneMapSingle ? 0 : 0,
    ...
    methods: {
        ...
        handleDrawPolygon() {
            ...
            drawPolygon(this.map, this.shapeOption, (finalPoints, DRAWPOLYGON, allArea) => {
                // 绘制结束后执行：保存顶点列表 + 显示面板
    ...
    watch: {
        currentTask: {
            async handler(newVal) {
                this.handleRemoveAllLayers();
                this.annotationList = [];
                this.activeToolIndex = this.oneMapSingle ? 0 : 1;
                this.isCollapsed = false;
                if (newVal.imageId) {
                    await this.initAnanotationList();
                }
            }
        },
        activeItem: {
            async handler(newVal) {
                ...
                this.isCollapsed = false;
                if (newVal.clue_id) {
                    await this.initClueViewPageAnnotationList();
                }
            }
        },
        activeToolIndex: {
...
::v-deep .el-dialog__header,
[data-v-28f795ec] .el-dialog__header {
    text-align: center;
    font-weight: 700;
