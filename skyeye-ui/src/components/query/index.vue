    ...
    <template v-for="(item, i) in config">
      <a-form-item :label="item.label" :key="i" class="input-container">
        <a-input
          ...
          v-decorator="item.decorator" allow-clear placeholder="请输入任务名称"
        ></a-input>
        <a-date-picker
          v-else-if="item.type === 'date'"
          placeholder="请选择日期"
          v-decorator="item.decorator"
          format="YYYY-MM-DD" allowClear
        >
        </a-date-picker>
          <a-select
                  v-else-if="item.type === 'taskselect'"
                  v-decorator="item.decorator"
                  :options="task_type_list" allowClear placeholder="请选择任务类型"
          ></a-select>
        <a-select
          v-else-if="item.type === 'select'"
          v-decorator="item.decorator"
          :options="countyList" allowClear placeholder="请选择检测区域"
        ></a-select>
      </a-form-item>
    </template>
