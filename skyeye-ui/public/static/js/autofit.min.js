...
const autofit = {
	...
	init(options = {}, isShowInitTip = true) {
		...
		const {
			dw = 1920, dh = 1080, el = typeof options === "string" ? options : "body", resize = true, ignore = [], transition = "none", delay = 0, limit = 0.1,
		} = options;
		currRenderDom = el;
...
function keepFit(dw, dh, dom, ignore, limit) {
	...
	for (let item of ignore) {
		...
		ignoreStyleDOM.innerHTML += `\n${itemEl}{transform:scale(${realScale})!important;transform-origin:0 0;width:${realWidth}!important;height:${realHeight}!important}`;
		if (realFontSize) {
