...
def merge_tif_and_convert_to_shp(input_path, output_path, img_name, fragment, regions_count):
    ...
    prjtag = 3
    for file in os.listdir(input_path):
        logger.info(r"栅格转矢量:已找到数据，开始每个分区的栅格栅格转面…………")
        # if "{}_reg".format(img_name) in file and file.endswith('.tif'):
        if img_name in file and "reg" in file and file.endswith('.tif'):
            file_path = os.path.join(input_path,file)
            save_path = os.path.join(os.path.dirname(os.path.dirname(file_path)),'shp','{}.shp'.format(os.path.splitext(os.path.basename(file_path))[0]))
            prjtag = raster_to_shp_smooth_elininate_fragment(fragment, file_path, save_path)
            input_list.append(save_path)
    logger.info(r"栅格转矢量:完成栅格转矢量及矢量优化处理")
    return prjtag,input_list
...
if __name__ == '__main__':
    start_datetime = datetime.fromtimestamp(time.time())
    input_path = r'C:\Users\Administrator\Desktop\test\tif'
    output_path = r'C:\Users\Administrator\Desktop\test\tif\1\all.shp'
    image_name = 'I50D005007'
    fragment = '0'
    regions_count = 3
