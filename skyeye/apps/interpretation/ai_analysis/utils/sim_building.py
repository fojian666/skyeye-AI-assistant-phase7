...
class Vector:
    ...
    def equals(self, v):
        # print( self.x1,v.x1,self.y1,v.y1,self.x2,v.x2,self.y2,v.y2 )
...
def CreateVectorFile(strVectorFile, sourceData):
    ...
    #srs=spatialref,
    layer = ds.CreateLayer(os.path.basename(strVectorFile)[:-4], srs=spatialref,geom_type=geomtype)
    for fd in fieldlist:
...
def RecSimilar(polygon):
    ...
    if ((pArea < 100 and areaDiff < 0.25) or (pArea >= 100 and pArea < 300 and areaDiff < 0.2) or (
            pArea >= 300 and pArea < 500 and areaDiff < 0.15)
            or (pArea >= 500 and pArea < 1000 and areaDiff < 0.1) or (pArea >= 1000 and pArea < 2000 and areaDiff < 0.1)
            or (pArea >= 2000 and pArea < 5000 and areaDiff < 0.05) or (pArea >= 5000 and areaDiff < 0.02)):
        ring = ogr.Geometry(ogr.wkbLinearRing)
        # opolygon = ogr.Geometry(ogr.wkbPolygon)
        for x, y in minRect.exterior.coords:
            ring.AddPoint(x, y)
        # opolygon.AddGeometry(ring)
        return 1, ring
...
def Ring2VList(ring):
    ...
    for i in range(ptCount):
        if i < ptCount - 1:
            if (ring.GetX(i) == ring.GetX(i + 1) and ring.GetY(i) == ring.GetY(i + 1)) == False:
                vList.append(Vector(ring.GetX(i), ring.GetY(i), ring.GetX(i + 1), ring.GetY(i + 1), i))
                # print(ring.GetX(i),ring.GetY(i),ring.GetX(i+1),ring.GetY(i+1),i)
    return vList
...
def VListDiret(vList):
    ...
    for i in range(0, 90, 1):
        ...
        for v in vList:
            angle = vx.cos(v)
            angleList = [math.fabs(90 - angle), math.fabs(180 - angle), math.fabs(angle)]
...
def FeatureLine2vList(vList):
    ...
    for i in range(len(vList)):
        ...
        if vList[i].x2 != vList[k].x1 or vList[i].y2 != vList[k].y1:
            ...
            ovList.append(Vector(vList[i].x2, vList[i].y2, vList[k].x1, vList[k].y1, i * vList[i].x2))
        else:
            ovList.append(vList[i])
    return ovList
...
def IntersectPt(v1, v2, vx):
    ...
    xc = (v1.x2 + v2.x1) / 2
    yc = (v1.y2 + v2.y1) / 2
    if math.sqrt((x - xc) ** 2 + (y - yc) ** 2) > math.sqrt((x1 - xc) ** 2 + (y1 - yc) ** 2):
        x = x1
        y = y1
    return x, y
    ...
    while not end:
        ...
        for i in range(len(vList)):
            ...
            if min([angle, math.fabs(180 - angle)]) < angleDif:
                ...
                break
            # 趋于垂直
            if math.fabs(90 - angle) < angleDif - 5 and math.fabs(
                    90 - angle) > 1 and v1.length() < lengthDif and v2.length() < lengthDif:
                # print(math.fabs(90 - angle))
                v1Angle = v1.cos(vx)
                v2Angle = v2.cos(vx)
                # print(math.fabs(90 - v1Angle))
                # print(math.fabs(90 - v2Angle))
                if (math.fabs(90 - v1Angle) < angleDif and math.fabs(90 - v1Angle) > 1) or (
                        math.fabs(90 - v2Angle) < angleDif and math.fabs(90 - v2Angle) > 1):
                    x, y = IntersectPt(v1, v2, vx)
                    # CreatePt(ptlayer, x, y)
                    vList.remove(v1)
                    vList.remove(v2)
...
def Domain(v1, v2, angleDiff, area, lengthDiff):
    ...
    if a1 < angleDiff and a1 < a2 and a1 < a3:
        # print(0, a1)
        return 0, a1
    if a2 < angleDiff - 5 and a2 < a1 and a2 < a3 and ((v1.x2 != v2.x1 and v1.y2 != v2.y1) or a2 >= 0.01):
        if (area >= 200 and v1.length() < lengthDiff and v2.length() < lengthDiff) or \
                area < 200:
            return 90, a2
        else:
...
def FeatureLine(vList, vx, angleDiff, lengthdiff1, lengthdiff2):
    ...
    for i in range(len(vList)):
        ...
        if (minAngle < angleDiff and v1.length() > lengthdiff1) or (v1.length() > lengthdiff2):
            avList.append(v1)
...
def ParaDistance(v1, v2, vx, lengthDiff, area, lengthdiff2):
    ...
    if ((vp1.length() == 0 or vp2.length() == 0) and d1 + d2 > lengthDiff):  # 重合的情况
        return 0, 0, 0, 0, -1

    if d1 + d2 <= lengthDiff or v12.length() < lengthDiff / 1.5:
        if (area >= 100 and v1.length() < lengthdiff2 and v2.length() < lengthdiff2) or \
                area < 100:
            return 0, 0, 0, 0, 0
        else:
            return 0, 0, 0, 0, 3
    ...
    if d1 + d2 > lengthDiff:
        if (area >= 200 and v1.length() < lengthdiff2 and v2.length() < lengthdiff2) or \
                area < 200:
    ...
    while not end:
        ...
        for i in range(len(vList)):
            ...
            if aV.domain == 901:
                ...
                x, y = line_intersection(((v1.x1, v1.y1), (v1.x2, v1.y2)), ((v2.x1, v2.y1), (v2.x2, v2.y2)))
                ov1 = Vector(v1.x1, v1.y1, x, y, v1.index)
                ov2 = Vector(x, y, v2.x2, v2.y2, v2.index + 1)
                vList.remove(v1)
                vList.remove(v2)
            ...
            if (aV.domain == 0 or aV.domain == 180):
                x1, y1, x2, y2, d = ParaDistance(v1, v2, vx, lengthdiff1, area, lengthdiff2 * 3)
                if d == -1:
                    continue
                if d == 1:  # 大于
        ...
        vl_num=len(vList)
        vList = FeatureLine(_list, vx, angleDiff, lengthdiff1 + 0.5, lengthdiff2)  # 最小，特征边
    ...
    for i in range(igeom.GetGeometryCount()):
        ...
        for i in range(ptCount):
            if iring.GetX(i) < 180:
                ...
                x, y = lonLat_2_xy(iring.GetX(i), iring.GetY(i))
            else:
...
def TranformPrjBack(igeom):
    ogeom = ogr.Geometry(ogr.wkbPolygon)  #创建一个面
    for i in range(igeom.GetGeometryCount()):
        iring = igeom.GetGeometryRef(i)
        oring = ogr.Geometry(ogr.wkbLinearRing)
        ptCount = iring.GetPointCount()
        for i in range(ptCount):
            x, y = xy_2_lonLat(iring.GetX(i), iring.GetY(i))
            oring.AddPoint(x, y)
        oring.CloseRings()
        ogeom.AddGeometry(oring)
    return ogeom

def lonLat_2_xy(lon, Lat):
    """
    该函数用于将经纬度坐标转换为投影坐标
    :param lon: 点的经度坐标
    :param Lat:点的纬度坐标
...
def ReconstructGeom(inputPath,outputPath,tempath,prjFlag):
    ...
    '''
    if not CheckFieldExist(ilayerDefn, "TYPE"):
        print("数据中没有TYPE字段，请检查")
        return []
    '''
    ifeatureCount = ilayer.GetFeatureCount()
    print("图层包含【%d】个图斑要素！" % ifeatureCount)
    ...
    for i in range(0, ifeatureCount):
        ...
        # print("正在处理value为【%d】的要素" % ifeatureOid)
        if r == 0 or r % 3000 == 0:
        ...
        aa=igeom.Area()
        #print(aa)
        if igeom.Area() < minArea or igeom is None :  # 空要素
            continue
        ...
        for j in range(iringCount):
            ...
            ideleteIds = []  # 对已经平滑过的要素再次平滑报错
            ideleteIds = compress(iringpts[0], iringpts[len(iringpts) - 1], iringpts, ideleteIds)
            ...
            if dpolygon.Boundary() == None:
                # 空图形
                continue
            averageLength = dpolygon.Boundary().Length() / dpolygon.GetGeometryRef(0).GetPointCount()
            averageLength = averageLength * 1.5
            area = dpolygon.Area()

            '''
            #是否圆形
            if CirSimilar(dpolygon)==1:
              cring=CreateCircle(dpolygon)
              opolygon.AddGeometry(cring)
              continue
            '''
            ...
            if dring.GetPointCount() == 4:  # 平滑后形成三角形
                new = 1
        ...
        if new == 1:
            ...
            # print(oupolygon.GetGeometryCount())
...
if __name__ == '__main__':
    ...
    input_path = r"E:\geo_ai_server\c#_test_data\result\6\change_detection\LD_2021_clip\LD_2021_clip\shp\Export_Output.shp"
    result_path = r"E:\geo_ai_server\c#_test_data\result\6\change_detection\LD_2021_clip\LD_2021_clip\shp\Export_Output1.shp"
    tempath = r"E:\geo_ai_server\c#_test_data\result\6\change_detection\LD_2021_clip\LD_2021_clip\shp\temp"
    prjFlag = 0
    # input_path = sys.argv[1]
    # result_path = sys.argv[2]
    # tempath = sys.argv[3]
    # prjFlag=int(sys.argv[4])
    sim_main(input_path, result_path, tempath, prjFlag)
    time_end = time.time()
    print("建筑物规则化耗时:%s" % time_interval(time_end, time_begin))
