...
class HighResolutionNet_OCR(nn.Module):
    def __init__(self, extra=None, **kwargs):
        ...
        self.aux_head = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(last_inp_channels, 1, kernel_size=1, stride=1, padding=0, bias=True))
    ...
    def forward(self, x):
        ...
        result = torch.cat([out, out_aux], 1)
        result = F.interpolate(result, size=(256, 256),
                           mode='bilinear', align_corners=ALIGN_CORNERS)
...
def get_seg_model(cfg, **kwargs):
    model = HighResolutionNet_OCR(cfg, **kwargs)
...
if __name__ == "__main__":
    ...
    model = HighResolutionNet_OCR(EXTRA)
