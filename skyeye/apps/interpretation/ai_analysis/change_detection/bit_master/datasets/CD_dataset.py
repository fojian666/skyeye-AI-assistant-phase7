...
class CDDataset_train(ImageDataset):

    def __init__(self, root_dir, img_size, split='train', is_train=True, label_transform=None,
                 to_tensor=True):
        super(CDDataset_train, self).__init__(root_dir, img_size=img_size, split=split, is_train=is_train,
                                        to_tensor=to_tensor)
...
class CDDataset_predict(ImageDataset):
    ...
    def __init__(self, root_dir, img_size, split='predict', is_train=False, label_transform=None,
                 to_tensor=True):
    ...
    def __getitem__(self, index):
        ...
        # if self.label_transform == 'norm':
        #     label = label // 255
        print("img的形状是：{}".format(img.shape))
        [img, img_B] = self.augm.transform_predict([img, img_B], to_tensor=self.to_tensor)
        # print(label.max())
        print("转换后img的形状是：{}".format(img.shape))
        print(1,img)
