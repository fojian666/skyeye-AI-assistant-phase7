...
import numpy as np
import torch,time,cv2,os,shutil
from math import *
import torch.nn as nn
from torchvision import models
from image_segmentation.networks.dinknet import DinkNet101, DinkNet50
import torch.nn.functional as F
from functools import partial
from PIL import Image
from  apps.interpretation import ai_config as cg
...
def get_dinknet(weight_path):
    ...
    if os.path.basename(weight_path).find("multi") != -1 or os.path.basename(weight_path).find("Multi") != -1:
        net = DinkNet101(num_classes=6).cuda()  # 多分类
    else:
...
def predict_binary_th_single(model, image_path_list, sum_pic_num, col, pic_num_per_row, area_info):
    ...
    for img_path in image_path_list:
        ...
        # predict = np.array(predict > 0.5, np.int)
...
def save_log(log_info, save_path):
    """
    保存日志文件
    Args:
        log_info: 日志信息
...
def segmentation_predict(tiff_file_input_path, output_path, model_path, img_name, fragment, building_regular,
                         base_progress, sum_progress, model, status_num_txt_path):
    """
    影像分割预测
    Args:
    ...
    Returns:
        result_shp_path: 结果shp路径
    """
    # 预测图片单元大小
    global img_name_tag
    img_name_tag = img_name
    logger.info("影像分割:开始运行@{}".format(img_name_tag))
    ...
    if not os.path.exists(tiff_file_input_path):
        return False
    update_status_bar(base_progress + 0.01 * 5 * sum_progress,status_num_txt_path)
    # 更新日志内容
    ...
    split_info, sum_col, sum_row, split_col, split_row = tiff_regions_meta_info_precalculating(tiff_file_input_path,
                                                                                               block_size,
                                                                                               root_temp_path)
    update_status_bar(base_progress + 0.01 * 10 * sum_progress,status_num_txt_path)
    ...
    update_status_bar(base_progress + 0.01 * 15 * sum_progress,status_num_txt_path)
    logger.info("加载模型:读取影像信息@{}".format(img_name_tag))
    # 面向影像文件的分区、切片、预测、每个分区切片的合并
    logger.info("影像分割预测:开始影像预测流程@{}".format(img_name_tag))
    update_status_bar(base_progress + 0.01 * 80 * sum_progress,status_num_txt_path)
    ...
    update_status_bar(base_progress + 0.01 * 95 * sum_progress,status_num_txt_path)
...
def region_merge_shp_gis_optimize_core(fragment, output_path, regions_count, img_name, building_regular):
    ...
    if building_regular == '1' and len(file_pathlist) != 0 and prjtag != 3:
        ...
        for i in file_pathlist:
            sim_building_path = os.path.join(output_path, "shp","{}_sim.shp".format(os.path.splitext(os.path.basename(i))[0]))
...
def splitregions_splitpieces_predict_picecesmerge_core(block_size, tiff_file_input_path, dlinknet_model, model_path,
                                                       ...
                                                       split_info, sum_col, sum_row, img_name, sum_progress):
    """
    ...
    SPLIT_STATUS_BAR_UNIT = int((65 * 0.01 * sum_progress) / sum_count_of_split_region)
    parm_list = []
    # 开始分区处理影像
    ...
    reqs = threadpool.makeRequests(multi_predict_dlinknet, parm_list)
    for req in reqs:
        pool.putRequest(req)
    pool.wait()
...
def multi_predict_dlinknet(parm):
    global img_name_tag
    SPLIT_STATUS_BAR_UNIT = parm[0]
    block_size = parm[1]
    current_region_index = parm[2]
    dlinknet_model = parm[3]
    img_name = parm[4]
    model_path = parm[5]
    output_path = parm[6]
    predict_time = parm[7]
    region_id = parm[8]
    regions_count = parm[9]
    root_temp_path = parm[10]
    split_info = parm[11]
    sum_col = parm[12]
    sum_row = parm[13]
    tiff_file_input_path = parm[14]

    current_region = split_info[region_id]
    current_region_index = current_region_index + 1
    logger.info('影像分割预测:'+"开始处理第" + region_id + "区" + "@{}".format(img_name_tag))
    # 用字符串存储当前处理区域索引和总区域数，用于在log中添加信息
    # message_str = "(第" + str(current_region_index) + "/" + str(regions_count) + "区)"
    message_str = ""
    # 如果分区了，会在图片名后加上索引号
    if len(split_info) > 1:
    ...
    logger.info('影像分割预测:开始切割影像' + message_str + "@{}".format(img_name_tag))
    # 运行切割影像的代码
    ...
    logger.info('影像分割预测:切割完成' + message_str + "@{}".format(img_name_tag))
    # 预测单片照片结果的存放位置
    ...
    # 多个切片预测结果的合并
    logger.info('影像分割预测:正在合并预测结果' + message_str + "@{}".format(img_name_tag))
    if class_type == "binary":
        # 二分类
    ...
    #     logger.info('影像分割预测:正在将预测结果转为8位' + message_str + "@{}".format(img_name_tag))
...
def change_detection_dlinknet(tiff_file_path, tiff_file_path2, save_path, model_path, tiff_name, tiff_name2,
                              fragment, building_regular,model,base_progress, sum_progress, status_num_txt_path,
                              folder_name):
    """
    用两次影像分割来做变化检测
    Args:
    ...
    shp1_path_list,prjtag = segmentation_predict(tiff_file_path, save_path, model_path, tiff_name, fragment,
                                     building_regular, base_progress, sum_progress*0.5,
                                     model,status_num_txt_path)
    shp2_path_list,prjtag = segmentation_predict(tiff_file_path2, save_path, model_path, tiff_name2, fragment,
                                     building_regular, (base_progress+sum_progress*0.5),
                                     sum_progress*0.5,model,status_num_txt_path)
    ...
    if len(merged_shp_gdf) > max_savenum:
        logger.info("栅格转矢量:超出存储上限，开始分块存储擦除结果")
        ...
        logger.info("栅格转矢量:未超上限，开始存储擦除结果")
...
if __name__ == '__main__':
    tiff_file_path = r'C:\Users\Administrator\Desktop\shp\remote_images\test1\202201_4490.tif'
    tiff_file_path2 = r'C:\Users\Administrator\Desktop\shp\remote_images\test2\202301_4490.tif'
    save_path = r'C:\Users\Administrator\Desktop\shp\remote_images\5'
    model_path = r'E:\geo_ai_server\gtrs_cs_server\model\DLinkNet_101-Road_Qinghai_2.th'
    tiff_name = '202201_4490'
    tiff_name2 = '202301_4490'
    pixel = '0.1'
    fragment = '150'
    building_regular = '1'
    region_path = ''
    ...
    change_detection_dlinknet(tiff_file_path, tiff_file_path2, save_path, model_path, tiff_name, tiff_name2, 
                              fragment, building_regular, model,
                              0, 90, status_num_txt_path, folder_name)