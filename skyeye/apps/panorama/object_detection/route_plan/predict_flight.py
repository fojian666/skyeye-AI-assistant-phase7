...
class GA(object):
    ...
    def greedy_init(self, dis_mat, num_total, num_city):  # 从0点开始找一条最短路径 （什么方法，从当前点到下一最短点的路径）
        ...
        for i in range(num_total):
            ...
            if start_index >= num_city - 1:
                # start_index = np.random.randint(1, num_city)
                start_index = np.random.randint(1, min(num_city, i))
    ...
    def run(self):
        try:
            ...
            self.txt_name = os.path.join(WORK_DIR, 'LAIS-AI/logs', self.uuid_value + '.txt')
            # 如果文件不存在则创建
            if not os.path.exists(self.txt_name):
                with open(self.txt_name, 'w', encoding='utf-8') as f:
                    f.write("")  # 创建空文件
                print(f"已创建日志文件: {self.txt_name}")
            else:
                print(f"日志文件已存在: {self.txt_name}")
...
def lonlat2geo(dataset, lon, lat):  #经纬度转平面

    prosrs, geosrs = getSRSPair(dataset)
    geosrs.SetWellKnownGeogCS("EPSG:4326")
    #4490转4548
    ct = osr.CoordinateTransformation(geosrs, prosrs)
    coords = ct.TransformPoint(lat, lon,0)
    xy=[coords[0],coords[1]]
    return xy

def geo2lonlat(dataset, x, y):   #平面转经纬度
...
def predict_flight_points(path,start_point,end_point,uuid_value):
    ...
    # fish_pt = os.path.join(os.path.dirname(os.path.join(__file__)), "create_fishNet_point.py")
    # os.system(
    #     python2_path + " " + fish_pt + " " + path + " " +prj_path+" "+ xls_path )
    # if not os.path.exists(xls_path):
    #     os.system(
    #         python2_path + " " + fish_pt + " " + path + " " + prj_path + " " + xls_path)
    get_finish_net_point(path, xls_path, prj_path)
    ...
    # data = read_excel_to_data(xls_path, start_point, end_point)
    print("读取excel")
    data = read_excel_xlsx_to_data(xls_path, start_point, end_point)
    # print(data)
    # 将读入数据写入并将路线初始化
    data = np.array(data)
    data = data[:, :-1]
    ...
    # GA对象初始化，
    model = GA(num_city=data.shape[0], num_total=200, iteration=10, data=data.copy(),uuid_value=uuid_value)
...
if __name__ == '__main__':
    ...
    path = r'C:\Users\Administrator\Desktop\tz_tbhz\tz_tbhz.shp'
